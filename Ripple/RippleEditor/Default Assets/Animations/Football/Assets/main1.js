...
function init() {
    ...
    boundaries.xEnd = 1720;
    boundaries.yStart = 25;
    boundaries.yEnd = 1000;
    goalPost.xStart = 1650;
    goalPost.xEnd = 1700; //not used
    goalPost.yStart = 330;
    goalPost.yEnd = 822;

    goalLines.top = getNewLine(1500, 225, 1500, 965);
    goalLines.bottom = getNewLine(1710, 240, 1770, 820);
    goalLines.left = getNewLine(1490, 185, 1910, 240);
    goalLines.right = getNewLine(1490, 1000, 1910, 770);

    ballMovingTimer = initializeTimer();
    timeKeeper = initializeTimer();
    setInterval(function () {
        if ((new Date().getTime() - mouseLastMoved) > 500) {
    ...
    netCanvas.onmousemove = function (e) {
        ...
        if (!ballMoving && !goalReached) {
            //If mean of last 50 mouse points is near to 25th mouse point 
            ...
            if (collision(e.clientX, e.clientY)) {
                ...
                var speed = 3 * (((distance(startPoint, start)) / 100) / timeKeeper.getTime());
...
function writeCenterText(message) {
    ...
    ctx.fillText(message, (actualXStart * fieldWidthRatio), ((600 - (startSize / 2)) * fieldHeightRatio));
...
function collision(mouseX, mouseY) {
    ...
    if (((mouseX > (ballX - 50)) && (mouseX < (ballX + 150))) && ((mouseY > (ballY - 50)) && (mouseY < (ballY + 150)))) {
...
function notifyWPF() {
    ...
    configureScreen("Assets/field.jpg", "Microsoft IT", "#fff");
}
...
function configureScreen(backgroundImage, centerMessage, textColor) {
    ...
    field.onload = function () {
        ...
        ctx.drawImage(ball, (point1.x) * fieldWidthRatio, (point1.y) * fieldHeightRatio, 100 * scrMul, 100 * scrMul);
        netCtx.drawImage(net, 1480 * fieldWidthRatio, 185 * fieldHeightRatio, (1910 - 1480) * fieldWidthRatio, (1015 - 185) * fieldHeightRatio);
...
function move(start, destination, speed) {
    ...
    if (distance(start, destination) > (10 / speed)) {
        ...
        var scrMul = window.innerHeight / 800;
        ctx.drawImage(ball, (destination.x), (destination.y), 100 * scrMul, 100 * scrMul);
        ballMoving = false;
        currentIndex += 1;
        speedCounter = 0;
...
function goalSideTouch(ballPosition, movementPoints) {
    ...
    if ((dotLineLength(currentPoint.x, currentPoint.y, (goalLines.left.xStart + 20) * fieldWidthRatio, (goalLines.left.yStart + 80) * fieldHeightRatio, goalLines.left.xEnd * fieldWidthRatio, goalLines.left.yEnd * fieldHeightRatio, true) < 10) && previousPoint.y > currentPoint.y) {
        netGoal = true;
    }
    if ((dotLineLength(currentPoint.x, currentPoint.y, (goalLines.right.xStart + 20) * fieldWidthRatio, (goalLines.right.yStart - 80) * fieldHeightRatio, goalLines.right.xEnd * fieldWidthRatio, goalLines.right.yEnd * fieldHeightRatio, true) < 10) && previousPoint.y < currentPoint.y) {
        netGoal = true;
    }
    return netGoal;
