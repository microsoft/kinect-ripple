...
/*	angle1 = Math.atan((point2.y-point1.y)/(point2.x-point1.x));
	...
	ball.onload = function(){
		...
		fieldWidthRatio = (WIDTH/ field.width);
		fieldHeightRatio = (HEIGHT/ field.height);
		ctx.drawImage(ball, (point1.x)*fieldWidthRatio, (point1.y)*fieldHeightRatio,100,100);
	};	
...
function move(start, destination, speed){
	...
	if(distance(start,destination) > (10/speed)){
		...
		ctx.drawImage(ball, (destination.x), (destination.y),100,100);	
		ballMoving = false;
		currentIndex+=1;
		speedCounter = 0;
...
function goalSideTouch(ballPosition, movementPoints) {
    ...
    if ((dotLineLength(currentPoint.x, currentPoint.y, (goalLines.left.xStart+20) * fieldWidthRatio, (goalLines.left.yStart+10) * fieldHeightRatio, goalLines.left.xEnd * fieldWidthRatio, goalLines.left.yEnd * fieldHeightRatio, true) < 10) && previousPoint.y > currentPoint.y) {
        netGoal = true;
    }
