...
namespace RippleFloorApp.Utilities.KinectGestures.Segments.Bend
{

    class BendSegment1 : IRelativeGestureSegment
    {
        public static double HipCenterAverage = 0.00;
        public GesturePartResult CheckGesture(Skeleton skeleton)
        {
            //double HipLeftY=  skeleton.Joints[JointType.HipLeft].Position.Y;
            //double HipRightY = skeleton.Joints[JointType.HipRight].Position.Y;
            //double HipLeftZ = skeleton.Joints[JointType.HipLeft].Position.Z;
            //double HipRightZ = skeleton.Joints[JointType.HipRight].Position.Z;
            
            //double KneeLeftY = skeleton.Joints[JointType.KneeLeft].Position.Y;
            //double KneeRightY = skeleton.Joints[JointType.KneeRight].Position.Y;
            //double KneeLeftZ = skeleton.Joints[JointType.KneeLeft].Position.Z;
            //double KneeRightZ = skeleton.Joints[JointType.KneeRight].Position.Z;

            //double FootLeftY = skeleton.Joints[JointType.FootLeft].Position.Y;            
            //double FootRightY = skeleton.Joints[JointType.FootRight].Position.Y;
            //double FootLeftZ = skeleton.Joints[JointType.FootLeft].Position.Z;
            //double FootRightZ = skeleton.Joints[JointType.FootRight].Position.Z;

            //double LeftPerpA = KneeLeftY - FootLeftY;
            //double LeftPerpB = HipLeftY - KneeLeftY;
            //double RightPerpA = KneeRightY - FootRightY;
            //double RightPerpB = HipRightY - KneeRightY;

            //double LeftBaseA = KneeLeftZ - FootLeftZ;
            //double LeftBaseB = HipLeftZ - KneeLeftZ;
            //double RightBaseA = KneeRightZ - FootRightZ;
            //double RightBaseB = HipRightZ - KneeRightZ;

            //double LeftA = Math.Sqrt((Math.Pow(LeftBaseA,2) + Math.Pow(LeftPerpA,2)));
            //double LeftB = Math.Sqrt((Math.Pow(LeftBaseB, 2) + Math.Pow(LeftPerpB, 2)));
            //double LeftC = HipLeftY - FootLeftY;

            //double RightA = Math.Sqrt((Math.Pow(RightBaseA, 2) + Math.Pow(RightPerpA, 2)));
            //double RightB = Math.Sqrt((Math.Pow(RightBaseB, 2) + Math.Pow(RightPerpB, 2)));
            //double RightC = HipRightY - FootRightY;
