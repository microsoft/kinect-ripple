...
function inCollisionZone(point) {
    var currentZone = flowerZones[flowerIndex - 1];
    var scrMul = window.innerWidth / 1200;
    if (flowerIndex > 0 && ((point.x > (currentZone.xTranslated - (100 * scrMul))) && (point.x < (currentZone.xTranslated + (100 * scrMul)))) && ((point.y > (currentZone.yTranslated - (100 * scrMul))) && (point.y < (currentZone.yTranslated + 100 * scrMul)))) {
        //console.log("ZONED");
...
function changeFlowerColor(zone, globalAlpha, image) {
    ...
    var scrMul = (window.innerWidth / 1200);
    flowerCtx.drawImage(image, -100 * scrMul, -100 * scrMul, 200 * scrMul, 200 * scrMul);
    flowerCtx.restore();
}
...
function drawLockedFlower(zone) {
    ...
    flowerCtx.drawImage(flowerImage.locked, -100 * scrMul, -100 * scrMul, 200 * scrMul, 200 * scrMul);
