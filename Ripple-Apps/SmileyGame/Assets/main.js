...
function addEnemy() {
    ...
    if (randomNumber>0.33 && randomNumber<0.66) {
        enemy.faceColor = "rgb(244,119,55)";
    }
    else if (randomNumber < 0.33) {
        enemy.faceColor = "rgb(31,178,89)";
        enemy.speed += 1;
    }
    else {
        enemy.faceColor = "rgb(0,175,219)";
        enemy.speed += 2;
    }
    enemies.push(enemy);
}
...
function drawPlayer(x, y) {
    ...
    context.fillStyle = "rgb(255,241,0)";
    context.lineWidth = 4;
    context.beginPath();
    //100,75
...
function handleTick() {
    ...
    if (!gameDone) {
        ...
        gameCanvas.getContext("2d").strokeStyle = 'rgb(252,176,52)';
        gameCanvas.getContext("2d").fillStyle = 'rgb(252,176,52)';
        gameCanvas.getContext("2d").font = "72px Segoe UI Light";
...
function resetCanvas(message) {
    ...
    ctx.fillStyle = "rgb(238,33,123)";
    ctx.fillRect(0, 0, canvasWidth, canvasHeight);
    gameCanvas.getContext("2d").strokeStyle = 'rgb(31,178,89)';
    gameCanvas.getContext("2d").fillStyle = 'rgb(31,178,89)';
    gameCanvas.getContext("2d").font = "72px Segoe UI Light";
    gameCanvas.getContext("2d").textBaseline = "top";
    ctx.save();
    ctx.translate((gameCanvas.width / 2) - 72, (gameCanvas.height / 2) - (ctx.measureText(message).width/2));
